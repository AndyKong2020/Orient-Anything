...
- 任务方法:输入图像经原项目前后处理流程进入模型,核心 DINOv2 与 MLP 前向在 NPU 执行,输出物体朝向角度和坐标轴叠加图。
...
- CUDA 依赖:无必需 CUDA 路径;NPU 适配后优先使用 Ascend NPU device。
...
- [x] NPU 适配改动(device、torch_npu、禁用 CUDA 核等):入口导入 torch_npu;优先选择 `npu:0`;输入 `pixel_values` 显式迁移到 NPU;模型缓存、Gradio 监听地址和端口改为环境变量配置。
...
部署边界:核心模型前向在 NPU 执行,NPU 适配关注模型主体算子链路。
...
| 直接推理,不移除背景 | NPU 前向 + 角度解码 | azimuth 3.0,polar 0.0,rotation 1.0,confidence 0.9965,耗时 0.077s | 通过 |
| 直接推理,移除背景 | 背景移除 + NPU 前向 + 角度解码 | azimuth 355.0,polar 0.0,rotation 2.0,confidence 0.2945,耗时 0.553s | 通过 |
...
| 指标 | 数值 |
|---|---|
| 能否在 NPU 跑通 | 模型主体兼容,DINOv2-Large + MLP head 单卡 NPU 前向通过 |
| NPU 硬阻塞 | 无。已验证推理和单步反传范围内,模型主体未发现阻断运行的 NPU 算子问题 |
| NPU 利用率 (npu-smi) | 100 step 模型前向采样峰值 93%,14 次轮询平均 6.64% |
| HBM 占用 | 100 step 模型前向采样峰值 7466MB / 114688MB,平均 6672MB |
| 模型主体关键算子 | profile 捕获 NPU kernel,未见阻断运行的算子问题 |
| 性能(吞吐/时延) | profiler 100 step 均值 11.139ms/图,约 89.8 img/s;WebUI/API 0.611s(不移除背景) / 1.808s(移除背景) |

- 兼容性结论:模型主体在 Ascend NPU 上兼容。已验证的推理和单步反传路径覆盖 DINOv2-Large backbone 与 MLP head,未发现 NPU 算子硬阻塞。
...
口径:Ascend 950PR、PyTorch eager、float32 权重与激活、单图模型前向。

训练路径:公开仓库没有训练入口。全模型训练验证覆盖 DINOv2-Large 主干和 MLP head 的 forward、loss、backward、SGD step;head 训练验证覆盖冻结主干微调场景。torch_npu 在反传中给出 internal format 到 base format 的转换 warning,但没有中断训练步骤。
...
| 调度(head) | 中 | 端到端重要因素 | 采样窗口内 launch 51800 次 |
...
NPU 算子层面无硬阻塞。以下只列会阻断指标复现或完整训练复现的项目级事项。

| 阻塞点 | 原因 | 是否硬阻塞 | CANN/AscendC 替代方案 | 处理方式 |
|---|---|---|---|---|
...
## 6. 结论
- 运行方案(NPU / NPU+CPU / CPU):NPU 部署。DINOv2-Large backbone 与 MLP 预测头在 NPU 上跑通。
- 待办 / 风险:模型主体在 NPU 上兼容,未发现 NPU 硬阻塞。完成范围为单卡 NPU 功能复现、官方 Demo 角度对齐、模型代码单步训练链路和 profiler 亲和性分析,不包含完整训练复现或论文级指标复现。
