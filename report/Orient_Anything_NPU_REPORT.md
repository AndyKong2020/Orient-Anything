...
- 兼容性结论:已验证范围不是 NPU 全兼容。模型主体的 DINOv2-Large backbone 与 MLP head 在 NPU 执行;端到端应用保留 CPU/host 路径。原项目本身在 CPU/host 侧实现的预处理、背景移除、渲染和服务逻辑属于部署边界,不按 NPU 阻塞项计。
...
以下只列会阻断指标复现或完整训练复现的事项。原项目 CPU/host 侧逻辑属于部署边界和回退路径,不作为 NPU 阻塞项。
...
- 待办 / 风险:模型主体在 NPU 上具备可运行亲和性;预处理、输出解码、背景移除、渲染和 Web 服务按原项目 CPU/host 路径保留,属于端到端部署边界。完成范围为单卡 NPU 功能复现、官方 Demo 角度对齐、模型代码单步训练链路和 profiler 亲和性分析,不包含完整训练复现或论文级指标复现。
