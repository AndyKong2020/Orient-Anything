...
部署边界:模型前向在 NPU,图像预处理、rembg 背景移除、输出解码、PIL/NumPy 坐标轴渲染和 Gradio 服务仍在 CPU/host 侧。项目运行形态为 NPU+CPU 混合部署,不是全链路 NPU 原生化,也不是无回退路径部署。
...
| 指标 | 数值 |
|---|---|
| 能否在 NPU 跑通 | 模型主体可以,DINOv2-Large + MLP head 单卡 NPU 前向通过;端到端为 NPU+CPU 混合执行 |
| NPU 利用率 (npu-smi) | 100 step 模型前向采样峰值 93%,14 次轮询平均 6.64% |
| HBM 占用 | 100 step 模型前向采样峰值 7466MB / 114688MB,平均 6672MB |
| 关键算子是否回退 CPU | 模型主体 profile 未见 CPU 回退;端到端存在 CPU/host 路径:AutoImageProcessor、输出解码、rembg、PIL/NumPy 渲染、Gradio 服务 |
| 性能(吞吐/时延) | profiler 100 step 均值 11.139ms/图,约 89.8 img/s;WebUI/API 0.611s(不移除背景) / 1.808s(移除背景) |

- 兼容性结论:已验证范围不是 NPU 全兼容。模型主体的 DINOv2-Large backbone 与 MLP head 在 NPU 执行;端到端应用保留 CPU/host 兜底路径。
- 算子回退清单:AutoImageProcessor resize/crop/normalize、推理输出 `.cpu()` 后的 argmax/softmax/标量化、rembg/ONNXRuntime 背景移除、PIL/NumPy 坐标轴渲染、Gradio 服务在 CPU/host 侧;模型主体 profile 捕获 MatMulV3、BatchMatMulV3、Conv2DV2、LayerNormV4、SoftmaxV2、Gelu 等 NPU kernel。
...
| 解码 | D2H `cpu`,slice,`argmax`,`softmax`,scalar cast | host/head | CPU 回退 |
...
算子回退清单:
- 已知非 NPU 路径:AutoImageProcessor resize/crop/normalize、推理输出 `.cpu()` 后的解码、rembg/ONNXRuntime 背景移除、PIL/NumPy 坐标轴渲染、Gradio 服务。
...
|---|---|---|---|---|
| 全链路 NPU 原生化未实现 | 图像预处理、rembg 背景移除、输出解码、坐标轴渲染和 Web 服务仍依赖 CPU/host 库 | 不阻塞当前部署,阻塞纯 NPU 端到端声明 | 可替换为 AscendCL/ACL 图像预处理、NPU 侧解码与后处理、自定义渲染或服务侧拆分 | 保留 CPU/host 路径,模型主体继续走 NPU |
| 推理输出解码回退 CPU | 当前实现显式把 logits 转 CPU 后做 argmax、softmax 和标量化 | 不阻塞功能,影响纯 NPU 后处理和端到端时延口径 | 将 argmax/softmax/角度解码保留在 NPU,只在最终展示前做一次 D2H | CPU 解码路径已通过功能验证 |
| 背景移除依赖 ONNXRuntime/rembg | 背景移除不是 PyTorch NPU 图的一部分,端到端开启该功能时增加 CPU 侧耗时 | 不阻塞核心朝向估计,影响开启背景移除时的端到端性能 | 使用 Ascend 推理后端承载背景分割模型,或将背景移除作为独立可选前处理服务 | 关闭背景移除可走更短路径;开启时保留 CPU rembg |
| TTA 聚合未作为主路径验证 | 测试时增强包含多裁剪、离群值过滤和圆周均值,会增加 head/host 与搬运压力 | 不阻塞默认 WebUI/API 路径 | 将聚合算子改为 NPU 侧向量后处理,减少中间 D2H | 默认关闭 TTA,单图路径已验证 |
| 训练链路缺少正式入口 | 模型代码可执行单步 forward/backward/SGD,但仓内没有数据集加载、loss 汇总、checkpoint 保存和评测闭环 | 阻塞完整训练复现,不阻塞模型代码反传验证 | 补齐训练脚本后按 NPU 数据加载、混精、保存恢复和多步稳定性重新验证 | 保留模型代码单步反传验证结果 |
...
- 待办 / 风险:模型主体在 NPU 上具备可运行亲和性,但端到端不是纯 NPU 部署;仍存在预处理、输出解码、背景移除、渲染和 Web 服务的 CPU/host 回退。完成范围为单卡 NPU 功能复现、官方 Demo 角度对齐、模型代码单步训练链路和 profiler 亲和性分析,不包含全链路 NPU 原生化、完整训练复现或论文级指标复现。
